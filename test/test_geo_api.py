# coding: utf-8

"""

    No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)

    OpenAPI spec version: v2
    
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""


from __future__ import absolute_import

import os
import sys
import unittest

import swagger_client
from swagger_client.rest import ApiException
from swagger_client.apis.geo_api import GeoApi


class TestGeoApi(unittest.TestCase):
    """ GeoApi unit test stubs """

    def setUp(self):
        self.api = swagger_client.apis.geo_api.GeoApi()

    def tearDown(self):
        pass

    def test_count_get(self):
        """
        Test case for count_get

        
        """
        pass

    def test_count_postjson(self):
        """
        Test case for count_postjson

        
        """
        pass

    def test_find(self):
        """
        Test case for find

        
        """
        pass

    def test_find_by_ids(self):
        """
        Test case for find_by_ids

        
        """
        pass

    def test_get_distinct_values(self):
        """
        Test case for get_distinct_values

        
        """
        pass

    def test_get_field_info(self):
        """
        Test case for get_field_info

        
        """
        pass

    def test_get_geo_json_for_locality(self):
        """
        Test case for get_geo_json_for_locality

        
        """
        pass

    def test_get_paths(self):
        """
        Test case for get_paths

        
        """
        pass

    def test_get_settings(self):
        """
        Test case for get_settings

        
        """
        pass

    def test_get_settings_0(self):
        """
        Test case for get_settings_0

        
        """
        pass

    def test_is_operator_allowed(self):
        """
        Test case for is_operator_allowed

        
        """
        pass

    def test_query(self):
        """
        Test case for query

        
        """
        pass


if __name__ == '__main__':
    unittest.main()
